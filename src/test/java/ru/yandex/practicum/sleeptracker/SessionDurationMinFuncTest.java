...
import ru.yandex.practicum.sleeptracker.functions.SessionDurationMinFunc;
...
public class SessionDurationMinFuncTest {
    ...
    @Test
    void shouldCalcMinSessionDuration() {
        SessionDurationMinFunc sessionDurationMinFuncTest = new SessionDurationMinFunc();

        SleepAnalysisResult<?> result = sessionDurationMinFuncTest.apply(sessionListTest);

        assertEquals(480L, result.getValue());
        assertEquals("Минимальная продолжительность сессии (мин)",
                result.getMessage());
    }
    ...
    void shouldHandleEmptyList() {
        ...
        SleepAnalysisResult<?> result = sessionDurationMinFuncTest.apply(emptyList);
