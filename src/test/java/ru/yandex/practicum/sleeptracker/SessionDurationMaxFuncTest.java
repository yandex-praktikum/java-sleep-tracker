...
import ru.yandex.practicum.sleeptracker.functions.SessionDurationMaxFunc;
...
public class SessionDurationMaxFuncTest {
    ...
    @Test
    void shouldCalcMaxSessionDuration() {
        SessionDurationMaxFunc sessionDurationMaxFuncTest = new SessionDurationMaxFunc();

        SleepAnalysisResult<?> result = sessionDurationMaxFuncTest.apply(sessionListTest);

        assertEquals(600L, result.getValue());
        assertEquals("Максимальная продолжительность сессии (мин)",
                result.getMessage());
    }
    ...
    void shouldHandleEmptyList() {
        ...
        SleepAnalysisResult<?> result = sessionDurationMaxFuncTest.apply(emptyList);
